...
router.post('/user',checkAuth,(req,res,next)=>{ 
    ...
    pool.query(`INSERT INTO users (
      ...
      if (error) {
        ...
        console.log
        addRate(1, result.rows[0].id, 5, 1, title_id);
        addRate(1, result.rows[0].id, rate, rater_id, title_id);
...
router.post('/brand',checkAuth,(req,res,next)=>{ 
    ...
    pool.query(`INSERT INTO brand (
      ...
      if (error) {
        ...
        addRate(2, result.rows[0].id, 5, 1, null);
        addRate(2, result.rows[0].id, rate, rater_id, null);
        res.status(201).json({id:result.rows[0].id})
      }
    })
})
...
router.post('/company',checkAuth,(req,res,next)=>{ 
    ...
    pool.query(`INSERT INTO company (
      ...
      if (error) {
        ...
        addRate(3, result.rows[0].id, 5, 1, null);
        addRate(3, result.rows[0].id, rate, rater_id, null);
        res.status(201).json({id:result.rows[0].id})
      }
    })
})
